...
function getKuponOsszesKategoriaKep() {
    $path = __DIR__ . '/kupon_osszes_kategoria_kep.txt';
    if (!file_exists($path)) {
        return '';
    }
    return trim((string) file_get_contents($path));
...
$aktualisKuponKatId = isset($_GET['kuponkat']) ? intval($_GET['kuponkat']) : 0;
$aktualisKuponVarosRaw = trim((string)($_GET['varos'] ?? ''));
$aktualisKuponVarosNormalized = strtolower(trim($aktualisKuponVarosRaw));
...
if ($aktualisKatKod === 'kupon') {
    $kuponNow = date("Y-m-d H:i:s");
    $kuponBaseWhere = [
        "t.ajanlat_tipus = 'kupon'",
        "t.eladva = 0",
        "COALESCE(t.darabszam, 0) > 0",
        "(t.kezdes_idopont IS NULL OR t.kezdes_idopont = '' OR t.kezdes_idopont <= '$kuponNow')",
        "(t.lejarat_idopont IS NULL OR t.lejarat_idopont = '' OR t.lejarat_idopont > '$kuponNow')",
        shobidMarketTermekWhere($conn, 't', $aktivPiacKod),
    ];

    $kuponBaseWhereCategoryCount = $kuponBaseWhere;
    if ($aktualisKuponVarosNormalized !== '') {
        $kuponVarosEsc = $conn->real_escape_string($aktualisKuponVarosNormalized);
        $kuponBaseWhereCategoryCount[] = "LOWER(TRIM(COALESCE(t.varos, ''))) = '$kuponVarosEsc'";
    }

    if ($kuponKategoriakTablaVan && $kuponKategoriaIdOszlopVan) {
        $kuponKatRes = $conn->query("
            SELECT kk.id, kk.nev, kk.kep_url, COALESCE(COUNT(t.id), 0) AS db
            FROM kupon_kategoriak kk
            LEFT JOIN termekek t
                ON t.kupon_kategoria_id = kk.id
               AND " . implode(' AND ', $kuponBaseWhereCategoryCount) . "
            GROUP BY kk.id, kk.nev, kk.kep_url
            ORDER BY COALESCE(kk.sorrend, 9999) ASC, kk.nev ASC
        ");
        if ($kuponKatRes) {
            while ($kk = $kuponKatRes->fetch_assoc()) {
                $kuponAlkategoriak[] = [
                    'id' => intval($kk['id'] ?? 0),
                    'nev' => function_exists('shobidI18nCouponCategoryName')
                        ? shobidI18nCouponCategoryName(
                            intval($kk['id'] ?? 0),
                            (string)($kk['nev'] ?? ''),
                            $pageLocaleCode
                        )
                        : (
                            function_exists('shobidI18nResolveMaybeKey')
                                ? shobidI18nResolveMaybeKey((string)($kk['nev'] ?? ''), $pageLocaleCode, (function_exists('st') ? st('feed.coupon.subcategories.unnamed', 'Kupon kategoria', $pageLocaleCode) : 'Kupon kategoria'))
                                : (string)($kk['nev'] ?? '')
                        ),
                    'kep_url' => trim((string)($kk['kep_url'] ?? '')),
                    'db' => intval($kk['db'] ?? 0),
                ];
            }
        }
    ...
    $kuponBaseWhereCity = $kuponBaseWhere;
    if ($aktualisKuponKatId > 0 && $kuponKategoriaIdOszlopVan) {
        $kuponBaseWhereCity[] = "t.kupon_kategoria_id = " . intval($aktualisKuponKatId);
    }

    $kuponVarosRes = $conn->query("
        SELECT MIN(TRIM(t.varos)) AS varos_megjelenites
        FROM termekek t
        WHERE " . implode(' AND ', $kuponBaseWhereCity) . "
          AND TRIM(COALESCE(t.varos, '')) <> ''
        GROUP BY LOWER(TRIM(t.varos))
        ORDER BY varos_megjelenites ASC
    ");
    if ($kuponVarosRes) {
        while ($vv = $kuponVarosRes->fetch_assoc()) {
            $label = trim((string)($vv['varos_megjelenites'] ?? ''));
            if ($label === '') {
                continue;
            }
            $kuponVarosValasztok[] = $label;
        }
    }
}
...
if ($id > 0) {
    ...
    if ($termek) {
        if (($termek['ajanlat_tipus'] ?? '') === 'kupon' && !empty($termek['kupon_kat_nev'])) {
            $termek['kat_nev'] = function_exists('shobidI18nCouponCategoryName')
                ? shobidI18nCouponCategoryName(
                    intval($termek['kupon_kategoria_id'] ?? 0),
	...
	if ($aktualisKatKod === 'kozelgo') {
        ...
        if ($aktualisKuponKatId > 0 && $kuponKategoriaIdOszlopVan) {
            $where[] = "t.kupon_kategoria_id = " . intval($aktualisKuponKatId);
        }
        if ($aktualisKuponVarosNormalized !== '') {
            $kuponVarosEsc = $conn->real_escape_string($aktualisKuponVarosNormalized);
            $where[] = "LOWER(TRIM(COALESCE(t.varos, ''))) = '$kuponVarosEsc'";
        }
    } elseif ($aktualis_kat > 0) {
    ...
    if ($res) {
        while ($row = $res->fetch_assoc()) {
            if (($row['ajanlat_tipus'] ?? '') === 'kupon' && !empty($row['kupon_kat_nev'])) {
                $row['kat_nev'] = function_exists('shobidI18nCouponCategoryName')
                    ? shobidI18nCouponCategoryName(
                        intval($row['kupon_kategoria_id'] ?? 0),
...
<div class="auction-shell">
  ...
  <main class="auction-feed-main">
    ...
    <div class="auction-feed-scroll" id="feed-scroll-area">
      ...
      <?php if ($aktualisKatKod === 'kupon'): ?>
      <section class="auction-category-strip-shell" id="kupon-subcategories">
        <button type="button" class="auction-category-strip__arrow is-left" onclick="scrollCategoryStrip(-1, 'auction-kupon-sub-strip')" aria-label="<?php echo htmlspecialchars(st('feed.coupon.subcategories.left', 'Kupon alkategoriak balra', $pageLocaleCode), ENT_QUOTES, 'UTF-8'); ?>"><img class="auction-arrow-icon" src="/kepek/icons/left.webp" alt=""></button>
        <div class="auction-category-strip" id="auction-kupon-sub-strip">
          <a class="auction-category-tile <?php echo $aktualisKuponKatId < 1 ? 'is-active' : ''; ?><?php echo !empty($kuponOsszesKategoriaKep) ? ' has-image' : ''; ?>" href="index.php?<?php echo http_build_query(array_filter(['kat' => 'kupon', 'q' => $keresesRaw !== '' ? $keresesRaw : null, 'varos' => $aktualisKuponVarosRaw !== '' ? $aktualisKuponVarosRaw : null])); ?>"<?php echo !empty($kuponOsszesKategoriaKep) ? ' style="background-image: url(/kepek/' . rawurlencode((string)$kuponOsszesKategoriaKep) . '), linear-gradient(45deg, #f5f5f505, #f5f5f525);"' : ''; ?>>
            <span class="auction-category-tile__count"><?php echo intval($kuponDb); ?></span>
            <span class="auction-category-tile__name"><?php echo htmlspecialchars(st('feed.coupon.subcategories.all', 'Osszes', $pageLocaleCode), ENT_QUOTES, 'UTF-8'); ?></span>
          </a>
          <?php foreach ($kuponAlkategoriak as $kk): ?>
          <a class="auction-category-tile <?php echo $aktualisKuponKatId === intval($kk['id']) ? 'is-active' : ''; ?><?php echo !empty($kk['kep_url']) ? ' has-image' : ''; ?>" href="index.php?<?php echo http_build_query(array_filter(['kat' => 'kupon', 'kuponkat' => intval($kk['id']), 'q' => $keresesRaw !== '' ? $keresesRaw : null, 'varos' => $aktualisKuponVarosRaw !== '' ? $aktualisKuponVarosRaw : null])); ?>"<?php echo !empty($kk['kep_url']) ? ' style="background-image: url(/kepek/' . rawurlencode((string)$kk['kep_url']) . '), linear-gradient(45deg, #f5f5f505, #f5f5f525);"' : ''; ?>>
            <span class="auction-category-tile__count"><?php echo intval($kk['db']); ?></span>
            <span class="auction-category-tile__name"><?php echo htmlspecialchars((string)$kk['nev']); ?></span>
          </a>
          <?php endforeach; ?>
        </div>
        <button type="button" class="auction-category-strip__arrow is-right" onclick="scrollCategoryStrip(1, 'auction-kupon-sub-strip')" aria-label="<?php echo htmlspecialchars(st('feed.coupon.subcategories.right', 'Kupon alkategoriak jobbra', $pageLocaleCode), ENT_QUOTES, 'UTF-8'); ?>"><img class="auction-arrow-icon" src="/kepek/icons/right.webp" alt=""></button>
      </section>

      <section class="auction-feed-header" style="padding-top:0.3rem;">
        <form method="GET" class="create-form-row" style="max-width:24rem;">
          <input type="hidden" name="kat" value="kupon">
          <?php if ($aktualisKuponKatId > 0): ?>
          <input type="hidden" name="kuponkat" value="<?php echo intval($aktualisKuponKatId); ?>">
          <?php endif; ?>
          <?php if ($keresesRaw !== ''): ?>
          <input type="hidden" name="q" value="<?php echo htmlspecialchars($keresesRaw); ?>">
          <?php endif; ?>
          <label class="create-label" for="kupon-varos-select"><?php echo htmlspecialchars(st('feed.coupon.city.label', 'Valassz varost', $pageLocaleCode), ENT_QUOTES, 'UTF-8'); ?></label>
          <select id="kupon-varos-select" class="create-select" name="varos" onchange="this.form.submit()">
            <option value=""><?php echo htmlspecialchars(st('feed.coupon.city.placeholder', '-- Valassz varost --', $pageLocaleCode), ENT_QUOTES, 'UTF-8'); ?></option>
            <?php foreach ($kuponVarosValasztok as $varosNev): ?>
            <option value="<?php echo htmlspecialchars($varosNev); ?>" <?php echo (strtolower(trim((string)$varosNev)) === $aktualisKuponVarosNormalized) ? 'selected' : ''; ?>><?php echo htmlspecialchars((string)$varosNev); ?></option>
            <?php endforeach; ?>
          </select>
        </form>
      </section>
      ...
      <section class="auction-card-grid">
        ...
        ?>
        <article class="auction-card" data-feed-item="<?php echo $feedCardIndex; ?>"<?php echo $feedCardIndex > 25 ? ' style="display:none;"' : ''; ?>>
      ...
      <?php if (count($ajanlatok) > 25): ?>
      <div id="feed-load-sentinel" style="height:1px;"></div>
      <?php endif; ?>
    </div>
...
<?php else: ?>
<script>
function scrollCategoryStrip(direction, stripId) {
  const targetId = stripId || "auction-category-strip";
...
(function initFeedProgressiveReveal() {
  const cards = Array.prototype.slice.call(document.querySelectorAll(".auction-card[data-feed-item]"));
  ...
  function showNextBatch() {
    ...
    loading = true;
    const nextLimit = Math.min(cards.length, visibleCount + batchSize);
